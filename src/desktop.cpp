...
void SPDesktop::init(SPNamedView *nv, Inkscape::UI::Widget::Canvas *acanvas, SPDesktopWidget *widget)
{
    namedview = nv;
    canvas = acanvas;
    _widget = widget;
    ...
    Inkscape::DrawingItem *drawing_item =
        document->getRoot()->invoke_show(*(canvas_drawing->get_drawing()), dkey, SP_ITEM_SHOW_DISPLAY);
    if (drawing_item) {
    ...
    _activate_connection = _activate_signal.connect(sigc::bind(sigc::ptr_fun(_onActivate), this));
    _deactivate_connection = _deactivate_signal.connect(sigc::bind(sigc::ptr_fun(_onDeactivate), this));

    _sel_modified_connection = selection->connectModified(sigc::bind(sigc::ptr_fun(&_onSelectionModified), this));
    _sel_changed_connection = selection->connectChanged(sigc::bind(sigc::ptr_fun(&_onSelectionChanged), this));
 ...
 * move_to_bottom = true by default so the item does not interfere with handling of other items on the canvas like
 * nodes.
 */
Inkscape::Display::TemporaryItem *SPDesktop::add_temporary_canvasitem(Inkscape::CanvasItem *item, guint lifetime,
                                                                      bool move_to_bottom)
{
    if (move_to_bottom) {
...
{
    // check for non-null temporary_item_list, because during destruction of desktop, some destructor might try to
    // access this list!
 ...
 */
void SPDesktop::set_active(bool new_active)
{
...
void SPDesktop::setEventContext(const std::string &toolName)
{
    ...
    if (toolName.empty()) {
        ...
        event_context->message_context =
            std::unique_ptr<Inkscape::MessageContext>(new Inkscape::MessageContext(this->messageStack()));
        event_context->setup();
 ...
 */
void SPDesktop::set_coordinate_status(Geom::Point p)
{
 ...
 */
void SPDesktop::zoom_absolute(Geom::Point const &center, double zoom, bool keep_point)
{
 ...
 * @param keep_point - Keep center fixed in the desktop window.
 */
void SPDesktop::zoom_relative(Geom::Point const &center, double zoom, bool keep_point)
{
 ...
 */
void SPDesktop::zoom_page()
{
    Geom::Rect d(Geom::Point(0, 0), Geom::Point(doc()->getWidth().value("px"), doc()->getHeight().value("px")));
 ...
 */
void SPDesktop::zoom_page_width()
{
...
{
    zoom_absolute(Geom::Point(doc()->getWidth().value("px") / 2, doc()->getHeight().value("px") / 2),
 ...
 */
void SPDesktop::flip_absolute_keep_point(Geom::Point const &c, CanvasFlip flip)
{
 ...
 */
void SPDesktop::flip_relative_keep_point(Geom::Point const &c, CanvasFlip flip)
{
    Geom::Point w = d2w(c); // Must be before flip.
 ...
 * @param flip Direction the canvas will be set as.
 */
void SPDesktop::flip_absolute_center_point(Geom::Point const &c, CanvasFlip flip)
{
    _current_affine.setFlip(flip);
    Geom::Rect viewbox = canvas->get_area_world();
    set_display_area(c, viewbox.midpoint());
 ...
 * @param flip Direction to flip canvas
 */
void SPDesktop::flip_relative_center_point(Geom::Point const &c, CanvasFlip flip)
{
    _current_affine.addFlip(flip);
    Geom::Rect viewbox = canvas->get_area_world();
    set_display_area(c, viewbox.midpoint());
 ...
 */
void SPDesktop::scroll_absolute(Geom::Point const &point, bool is_scrolling)
{
 ...
 */
void SPDesktop::scroll_relative(Geom::Point const &delta, bool is_scrolling)
{
 ...
 */
void SPDesktop::scroll_relative_in_svg_coords(double dx, double dy, bool is_scrolling)
{
...
bool SPDesktop::is_darktheme()
{
    return getToplevel()->get_style_context()->has_class("dark");
}

bool SPDesktop::is_maximized()
{
...
void SPDesktop::getWindowGeometry(gint &x, gint &y, gint &w, gint &h)
{
    _widget->getWindowGeometry(x, y, w, h);
}

void SPDesktop::setWindowPosition(Geom::Point p)
{
    _widget->setWindowPosition(p);
}

void SPDesktop::setWindowSize(gint w, gint h)
{
    _widget->setWindowSize(w, h);
}

void SPDesktop::setWindowTransient(void *p, int transient_policy)
{
 ...
 */
bool SPDesktop::onWindowStateEvent(GdkEventWindowState *event)
{
...
void SPDesktop::setToolboxFocusTo(gchar const *label)
{
    _widget->setToolboxFocusTo(label);
}

void SPDesktop::setToolboxAdjustmentValue(gchar const *id, double val)
{
    _widget->setToolboxAdjustmentValue(id, val);
}

Gtk::Toolbar *SPDesktop::get_toolbar_by_name(const Glib::ustring &name)
{
    return _widget->get_toolbar_by_name(name);
}

bool SPDesktop::isToolboxButtonActive(gchar const *id)
{
...
void SPDesktop::enableInteraction()
{
...
void SPDesktop::onResized(double /*x*/, double /*y*/)
{
    // Nothing called here
}
 ...
 * Redraw callback; queues Gtk redraw; connected by View.
 */
void SPDesktop::onRedrawRequested()
{
    if (_widget) {
...
void SPDesktop::updateCanvasNow()
{
...
void SPDesktop::setDocument(SPDocument *doc)
{
    ...
    if (canvas_drawing) {
        ...
        Inkscape::DrawingItem *drawing_item =
            doc->getRoot()->invoke_show(*(canvas_drawing->get_drawing()), dkey, SP_ITEM_SHOW_DISPLAY);
        if (drawing_item) {
            canvas_drawing->get_drawing()->root()->prependChild(drawing_item);
        }
...
void SPDesktop::onStatusMessage(Inkscape::MessageType type, gchar const *message)
{
    if (_widget) {
...
void SPDesktop::onDocumentURISet(gchar const *uri)
{
 ...
 * Resized callback.
 */
void SPDesktop::onDocumentResized(gdouble width, gdouble height)
{
...
void SPDesktop::_onActivate(SPDesktop *dt)
{
    if (!dt->_widget)
        return;
...
void SPDesktop::_onDeactivate(SPDesktop *dt)
{
    if (!dt->_widget)
        return;
...
void SPDesktop::_onSelectionModified(Inkscape::Selection *selection, guint /*flags*/, SPDesktop *dt)
{
    if (!dt->_widget)
        return;
 ...
 */
static bool _drawing_handler(GdkEvent *event, Inkscape::DrawingItem *drawing_item, SPDesktop *desktop)
{
...
static void _namedview_modified(SPObject *obj, guint flags, SPDesktop *desktop)
{
    ...
    if (flags & SP_OBJECT_MODIFIED_FLAG) {
        ...
        Inkscape::Preferences *prefs = Inkscape::Preferences::get();
        if (SP_RGBA32_R_U(nv->pagecolor) + SP_RGBA32_G_U(nv->pagecolor) + SP_RGBA32_B_U(nv->pagecolor) >= 384) {
