...
bool DialogManager::open_floating(unsigned int code) {
...
void DialogManager::restore_dialogs_state(DialogContainer* docking_container, bool include_transient) {
	if (!docking_container) return;
	...
	try {
		...
		if (keyfile->load_from_file(filepath)) {
			...
			docking_container->load_container_state(keyfile.get());
