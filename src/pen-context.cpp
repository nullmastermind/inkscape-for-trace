            ...
            case SP_PEN_CONTEXT_MODE_DRAG:
                switch (pc->state) {
                    ...
                    case SP_PEN_CONTEXT_POINT:
                        ...
                        if(((pc->spiro || pc->bspline) && pc->state != SP_PEN_CONTEXT_CLOSE)&&!(pc->bspline && bevent.state & GDK_SHIFT_MASK) ) pc->state = SP_PEN_CONTEXT_POINT;
                        //BSpline End
                        ret = TRUE;
                        break;
                    case SP_PEN_CONTEXT_CONTROL:
        ...
        case SP_PEN_CONTEXT_MODE_DRAG:
            switch (pc->state) {
                ...
                case SP_PEN_CONTEXT_CLOSE:
                    ...
                    //BSpline
                    if (!pc->polylines_only && !pc->spiro && !pc->bspline) {
                    //BSpline End
                        spdc_pen_set_ctrl(pc, p, mevent.state);
                    } else {
...
*/
        ...
        case GDK_KEY_KP_Delete:
            if ( pc->green_curve->is_empty() || (pc->green_curve->last_segment() == NULL) ) {
                ...
                // Reset red curve
                //BSpline
                if ((pc->spiro || pc->bspline) && pc->green_curve->get_segment_count()==1) {
                    pen_cancel (pc);
                    ret = TRUE;
                }
                //BSpline End
...
static void bspline(SPPenContext *const pc, bool Shift)
{
    if(!Shift){
        ...
        if (!pc->red_curve->is_empty()){
            pc->npoints = 5;
            pc->p[0] = pc->red_curve->first_segment()->finalPoint();
            pc->p[3] = pc->red_curve->first_segment()->initialPoint();
            pc->p[2] = pc->p[3];
        }
